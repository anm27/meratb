...
io.on('connection', (socket) => {
    ...
    socket.on('sendTravelRequest', (data) => {
        ...
        io.to(recipientId).emit('receiveTravelRequest', { senderId, pickupLocation, dropLocation });
    });
    ...
    socket.on('acceptTravelRequest', async (data) => {
        ...
        try {
          await TravelDetails.updateMany(
            ...
            { matchedAt: new Date(), expireAt: new Date(Date.now() + 2 * 60 * 60 * 1000) } // Set expireAt to 2 hours from now
          );
      
          console.log(`Travel request from ${senderId} accepted by ${recipientId}`);
          console.log(`Emitting requestAccepted event to ${recipientId} and ${senderId}`);
    ...
    socket.on('sendMessage', (data) => {
        ...
        jwt.verify(token, JWT_SECRET, (err, decoded) => {
            ...
            io.to(recipientId).emit('receiveMessage', { text: message, sender: senderId });
            io.to(senderId).emit('receiveMessage', { text: message, sender: senderId });
            console.log(`Emitted receiveMessage event to ${recipientId} with text: ${message} and sender: ${senderId}`);
        });
    });
