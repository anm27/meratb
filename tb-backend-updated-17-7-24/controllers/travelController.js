...
exports.submitDetails = async (req, res) => {
    try {
        const { pickupLocation, dropLocation, timeSlot } = req.body;
        ...
        const currentUserMatches = matches.filter(match => match.userId._id.toString() !== req.user.id);
        ...
        matches.forEach(match => {
            if (match.userId._id.toString() !== req.user.id) {
                const userMatches = matches.filter(m => m.userId._id.toString() !== match.userId._id.toString());
                console.log(`Emitting matches to user ${match.userId._id}:`, userMatches);
...
exports.sendTravelRequest = (req, res) => {
    ...
    req.io.to(recipientId).emit('receiveTravelRequest', { senderId, pickupLocation: req.body.pickupLocation, dropLocation: req.body.dropLocation });
