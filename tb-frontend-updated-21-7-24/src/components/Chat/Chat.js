...
const Chat = ({ recipientId }) => {
    ...
    const handleSendMessage = () => {
        ...
        setMessages((prevMessages) => [...prevMessages, { text: message, sender: user._id }]);
        setMessage('');
        handleTyping(false); 
    };
    ...
    const handleTyping = (isTyping) => {
        ...
        if (isTyping) {
            typingTimeoutRef.current = setTimeout(() => {
                socket.emit('typing', { recipientId, senderId: user._id, isTyping: false });
            }, 2000);
        }
    };

    return (
        <div className="min-h-screen bg-[#122620] text-white flex flex-col items-center p-4">
            <h2 className="text-2xl font-bold mb-4 text-[#fff]">Chat with {recipientId}</h2>
            {error && <p className="text-red-500 mb-4">{error}</p>}
            <div className="w-full max-w-lg bg-[#1E2D2B] rounded-lg shadow-lg p-4 flex flex-col space-y-4">
                <div className="flex flex-col space-y-2 overflow-auto h-64">
                    {messages.map((msg, index) => (
                        <div
                            key={index}
                            className={`p-2 rounded-md shadow-md ${msg.sender === user._id ? 'bg-[#8AC9A7] self-end text-black' : 'bg-[#2A3F3D] self-start text-white'}`}
                        >
                            <strong>{msg.sender === user._id ? 'You' : msg.sender}:</strong> {msg.text}
                        </div>
                    ))}
                </div>
                {typing && <p className="text-sm text-gray-400">{recipientId} is typing...</p>}
