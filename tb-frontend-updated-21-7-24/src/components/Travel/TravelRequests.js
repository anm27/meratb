...
const TravelRequests = ({ onRecipientSelected, onMatchedUsersCount, onTravelRequestCount, matchedUsersRef, travelRequestsRef }) => {
    ...
    useEffect(() => {
        if (user) {
            ...
            socket.on('requestAccepted', (data) => {
                ...
                setAcceptedRequests((prevAcceptedRequests) => [...prevAcceptedRequests, data]);
    ...
    const handleSendRequest = (recipientId) => {
        ...
        console.log('Sending travel request to:', recipientId, 'from:', senderId, 'with token:', token);
        socket.emit('sendTravelRequest', { recipientId, senderId, pickupLocation: "Your Pickup Location", dropLocation: "Your Drop Location" });
    ...
    const isRequestAccepted = (userId) => {
        return acceptedRequests.some(request => request.senderId === userId || request.recipientId === userId);
    };

    return (
        <div ref={matchedUsersRef} className="min-h-screen bg-[#122620] text-white p-8 ">
            <div className="max-w-4xl mx-auto">
                <h2 className="text-3xl font-bold mb-6 text-white text-center">Matched Users</h2>
                <ul className="lg:flex gap-2 justify-center">
                    ...
                    {matchedUsers.length > 0 ? (
                        matchedUsers.map(user => (
                            <li key={user._id} className="bg-[#1E2D2B] p-4 rounded-lg shadow-lg lg:w-1/3 md:w-1/2 grid justify-center items-center gap-2 py-3">
                                <div>
                                    <div className='tracking-wider'>
                                    <p className="text-lg font-medium">Username: {user.userId.username}</p>
                                    <p className="text-sm">Going to travel from {user.pickupLocation} to {user.dropLocation}</p>
                ...
                {selectedUser && (
                    <div ref={travelRequestsRef} className='flex justify-center items-center'>
                    <div className="mt-8 bg-[#1E2D2B] p-6 rounded-lg shadow-lg">
                        <h3 className="text-2xl font-bold mb-4 text-white">Send Travel Request to {selectedUser.userId.username}</h3>
                ...
                <Modal isVisible={showModal} onClose={() => setShowModal(false)}>
                    <div className='flex justify-center items-center'>
                        <div className="bg-[#1E2D2B] p-6 rounded-lg shadow-lg w-full text-center">
                            <h3 className="text-2xl font-bold mb-4 text-white">{modalMessage}</h3>
                            <button
                ...
                <h2 className="text-3xl font-bold mt-12 mb-6 text-white text-center">Received Requests</h2>
                <ul className="lg:flex grid gap-2 justify-center items-center">
                    {requests.map((request, index) => (
                        <li key={index} className="bg-[#1E2D2B] p-4 rounded-lg shadow-lg grid justify-center items-center">
                            <div>
                                <p className="text-lg font-medium">{request.senderId}</p>
                                <p className="text-sm">{request.pickupLocation} to {request.dropLocation}</p>
                            </div>
                            <div className="flex space-x-4">
